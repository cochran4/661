---
title: "Clinical trials as our exemplar"
---

For our first real foray into causal inference, we use clinical trials as our exemplar. Clinical trials show us how causal inference is possible with randomization. They also introduce a roadmap that we will follow throughout this textbook: **construction, identification, and estimation**.
...
![Six women from three age groups assigned to hormone therapy or placebo, with coronary heart disease recorded afterward.](images/01c-prototrial-whi-paperback.png){fig-align="center" width="80%"}

We use three variables to describe a patient in the study:

- **Age:** $X=0$ if 50-59 years of age, $X=1$ if 60-69, and $X=2$ if $70-79$.
- **Treatment assignment:** $A=1$ if estrogen plus progestin and $A=0$ if placebo
- **Outcome:** $Y=1$ if a coronary heart disease event occurs and $Y=0$ if not.

We treat $X$, $A$, and $Y$ as random variables. We also assume that the observations from different participants are independent and that every participant's observations come from the same probability model.
...
For both treatment-group means to be defined, each treatment must have a positive probability of being assigned. If everyone received hormone therapy, there would be no placebo group, so $E[Y\mid A=0]$ would not be defined. The same problem would arise for $E[Y\mid A=1]$ if everyone received placebo. This property is called [positivity]{.glossary-term}. In the WHI trial, randomization gave each participant a positive probability of assignment to hormone therapy and to placebo.

The difference between the two group averages estimates the [risk difference]{.glossary-term}:

$$
E[Y\mid A=1]-E[Y\mid A=0].
$$

Our strategy is not so bad. Because we assume participants provide independent observations from the same probability model, the sample average in each treatment group is [unbiased]{.glossary-term data-glossary="Unbiased estimator"} for the corresponding treatment-group mean.
...
![A common path splits into two parallel universes containing the same woman. In one universe, she is assigned estrogen plus progestin; in the other, she is assigned placebo. Her outcome in each universe is unknown.](images/01c-potential-outcomes-whi-paperback-v3.png){width="82%" fig-align="center"}
...
In mathematics, a definition is [well-defined]{.glossary-term data-glossary="Well-defined (mathematical sense)"} when each possible input determines one unambiguous object. Here, the treatment value $a$ must describe a sufficiently precise change to our causal story that the potential outcome $Y(a)$ has one clear meaning.

::: {.example-intro}
<span class="example-label">Example 1: Which value of $x$?</span>

Suppose we define $x$ to be the number satisfying

$$x^2=9.$$

This does not define one value of $x$. Both $x=3$ and $x=-3$ satisfy the equation. We could make the definition well-defined by saying that $x$ is the *positive* number satisfying $x^2=9$. That additional information tells us which value we mean.
:::

::: {.example-intro}
<span class="example-label">Example 2: The same nationality</span>
...
Well-definedness is also a consideration for a clinical trial. The intervention and comparator should be described clearly enough that we know which two scenarios we are comparing. This is one reason clinical trials have protocols. A drug protocol specifies such details as the medication, dose, timing, duration, and what to do after a missed dose or an adverse event. A behavioral intervention might specify the script a counselor follows, the number and length of sessions, and how the counselor responds to particular situations. The protocol turns a broad treatment label into an intervention that can be implemented consistently.

The WHI protocol did not simply say “give hormone therapy.” It specified a daily tablet containing 0.625 mg of conjugated equine estrogen plus 2.5 mg of medroxyprogesterone acetate, compared with a matching placebo. Using the same protocol across clinical centers helps ensure that assignment to hormone therapy represents the same intervention everywhere.

To see the problem more concretely, we consider several examples where potential outcomes might not be well-defined.

::: {.example-intro}
<span class="example-label">Example 3: Unclear medication schedule</span>
imagine instead that the WHI investigators simply handed each woman a bottle of hormone-therapy pills without explaining how to take them. One woman might take one pill each morning, another might take two pills, and another might take pills only when she experienced symptoms. These versions could produce different coronary-heart-disease outcomes. “Receive a bottle of pills” would therefore not determine one unambiguous potential outcome.
:::

::: {.example-intro}
<span class="example-label">Example 4: Grace periods</span>
A grace period provides a subtler example. Suppose the intervention is “initiate the medication sometime during the next seven days.” One person might initiate it today, while another might wait until the seventh day. If the timing of initiation could affect the outcome, those are relevant versions of the intervention. The protocol would need to specify when treatment is initiated (or specify a rule for choosing the initiation time) before the corresponding potential outcome has one clear meaning.
:::

::: {.example-intro}
<span class="example-label">Example 5: Interference</span>
There is another way in which specifying one person's intervention may not completely describe the relevant scenario. Suppose the outcome is whether I develop influenza and the intervention is whether I receive a flu vaccine. My outcome may depend not only on whether I am vaccinated but also on whether my husband is vaccinated. His vaccination could make him less likely to become infected and pass influenza to me. In that setting, “my outcome if I am vaccinated” does not specify everything that could affect my potential outcome.

This dependence of one person's potential outcome on another person's treatment is called **interference**. It commonly arises with contagious diseases, but it can also occur when interventions spread through households, schools, workplaces, or social networks. If one participant's assignment changes another participant's exposure, behavior, or access to resources, specifying only each person's own treatment may not define the relevant potential outcome.
:::

Some variation is natural even under a detailed protocol, and that is not automatically a problem. The bottles might differ in color, or one participant might take her tablet with water while another takes it with juice. If these versions would not change the potential outcome, the variation is irrelevant to the causal question. This condition is called **treatment-variation irrelevance**. What matters is that the treatment label not hide versions—such as different drugs, doses, schedules, or durations—that could produce meaningfully different potential outcomes.


<details class="lesson-question technical-proof">
<summary>Optional technical note: SUTVA</summary>

The **stable unit treatment value assumption** (SUTVA) is the traditional property used to make sure potential outcomes are well-defined [@rubin1980]:
...
In the WHI example, knowing the individual treatment effect would tell us whether the patient should or should not take hormone replacement therapy. An individual treatment effect of one would tell me that hormone replacement therapy is **not** a good idea for the patient. They get coronary heart disease under therapy, but not if they take the placebo. An individual treatment effect of -1 tells me hormone replacement therapy is a good idea. They get coronary heart disease under placebo, but not if they take hormone replacement therapy.

Calculating this effect requires both potential outcomes, but the other potential outcome is missing. This is called the [fundamental problem of causal inference]{.glossary-term}.
...
This difference is the [average treatment effect (ATE)]{.glossary-term}. For the WHI study, the ATE is the difference between the probability of coronary heart disease if everyone receives hormone therapy and the probability if everyone receives placebo. A positive value would mean that hormone therapy increases risk, a negative value would mean that it decreases risk, and a value of zero would mean that the risks are the same.

A [causal estimand]{.glossary-term} is a probabilistic quantity involving potential outcomes that we want to estimate. The ATE is our causal estimand for the WHI study.
...
We say that a causal estimand is [identified]{.glossary-term} when it can be written entirely in terms of observed variables and their probability model. Here, we have identified the average treatment effect by expressing the comparison between our two imagined universes as a comparison between the observed WHI treatment groups.

![Identification expresses a causal estimand as a statistical estimand that can be learned from observed data.](images/01c-causal-identification-path.svg){width="75%" fig-align="center"}
...
1. **Positivity** ensures that $P(A=a)>0$, so the mean among people assigned treatment $a$ is defined.
2. **Well-definedness** ensures that the potential outcome $Y(a)$ has one unambiguous meaning.
